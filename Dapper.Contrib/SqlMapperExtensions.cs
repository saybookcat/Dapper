...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, string> GetListQueries = new ConcurrentDictionary<RuntimeTypeHandle, string>();
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, string> ContainQueries = new ConcurrentDictionary<RuntimeTypeHandle, string>();
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, DynamicParameters> EntityParameter = new ConcurrentDictionary<RuntimeTypeHandle, DynamicParameters>();
        ...
        {
            var type = typeof(T);
            return GetSingleKey(type, method);
        ...
        /// <returns></returns>
        public static bool Contains<T>(this IDbConnection connection, T entity, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (entity == null)
            {
                return false;
            ...
            if (!ContainQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                if (type.IsArray)
                {
                    throw new DataException("Contains only support sigle data");
                }
                ...
                strExists.AppendFormat(" select 1 from {0}  ", name);
                ...
                if (isExplicit)
                {
                    ...
                    if (keyProperties.Count > 0)
                    {
                        ...
                        for (int i = 0; i < keyProperties.Count; i++)
                        {
                            ...
                            adapter.AppendColumnNameNotEqualsValue(strWhere, property.Name);
            ...
            object exists = null;
            exists = connection.ExecuteScalar(sql, entity, commandTimeout: commandTimeout, transaction: transaction);
            return exists != null;
        }
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                obj = connection.QueryFirstOrDefault<T>(sql, dynParms, transaction, commandTimeout: commandTimeout);
            }
            return obj;
        }
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static dynamic Get(this IDbConnection connection, Type type, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                var key = GetSingleKey(type, nameof(Get));
                var name = GetTableName(type);
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static T GetSigle<T>(this IDbConnection connection, object entity = null, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            string sql = "";
            var name = GetTableName(type);
            ...
            if (!type.IsInterface())
            {
                return connection.QueryFirstOrDefault<T>(sql, entity, transaction, commandTimeout: commandTimeout);
            }
            else
            {
                return connection.QueryFirstOrDefault(sql, entity, transaction, commandTimeout: commandTimeout);
            }
        }
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object entity = null, bool isasc = true, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            string sql = "";
            var name = GetTableName(type);
            ...
            if (!type.IsInterface())
            {
                return connection.Query<T>(sql, entity, transaction, commandTimeout: commandTimeout);
            }
            var result = connection.Query(sql, entity, transaction, commandTimeout: commandTimeout);
            var list = new List<T>();
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static IEnumerable<T> GetPage<T>(this IDbConnection connection, int pageIndex, int pageSize, object entity = null, bool isasc = true, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            ...
            if (!type.IsInterface())
            {
                return connection.Query<T>(sb.ToString(), param, transaction, commandTimeout: commandTimeout);
            }
            var result = connection.Query(sb.ToString(), param, transaction, commandTimeout: commandTimeout);
            var list = new List<T>();
            foreach (IDictionary<string, object> res in result)
            {
                var obj = ProxyGenerator.GetInterfaceProxy<T>();
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, bool isasc = true, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
        ...
        /// 获取行数
        /// </summary>
        public static int GetRowCount<T>(this IDbConnection connection, object entity = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            object enm = connection.ExecuteScalar(sb.ToString(), param, transaction, commandTimeout: commandTimeout);
            if (enm == null)
            {
                return 0;
            }
        ...
        /// <summary>
        /// get the tableName by tableType
        /// </summary>
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static bool DeleteByKey<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);

            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                var key = GetSingleKey<T>(nameof(DeleteByKey));
                var name = GetTableName(type);
        ...
        /// <typeparam name="T">Type of entity</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        /// <param name="ids">key in list</param>
        /// <param name="transaction">The transaction to run under, null (the default) if none</param>
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if deleted, false if not found</returns>
        public static bool DeleteList<T>(this IDbConnection connection, List<int> ids, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var adapter = GetFormatter(connection);
            var key = keyProperties[0];
            var name = GetTableName(type);

            var sb = new StringBuilder();
            sb.AppendFormat("delete from {0} where ", name);
            adapter.AppendColumnName(sb, key.Name);  //fix for issue #336
            sb.Append(" in ");
            sb.Append(" ( ");
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if deleted, false if not found</returns>
        public static bool Delete<T>(this IDbConnection connection, object entity, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            if (entity == null)
            {
                throw new ArgumentException("Cannot Delete null Object {0}", nameof(entity));
            }
            var type = typeof(T);
            ...
            if (param == null || param.ParameterNames.Count() == 0)
            {
                throw new ArgumentException("Object {0} Have Not Delete Arguments ", nameof(entity));
            }
            var name = GetTableName(type);
            ...
            var deleted = connection.Execute(sb.ToString(), entity, transaction, commandTimeout);
            return deleted > 0;
        }
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private static DynamicParameters GetParameters(object entity)
        {
            if (entity == null)
            {
                return null;
            ...
            var properties = type.GetProperties().Where(m => IsWriteable(m).Equals(true)).ToArray();
            foreach (var item in properties)
            {
                System.Reflection.PropertyInfo propertyInfo = type.GetProperty(item.Name); //获取指定名称的属性
                object obj = propertyInfo.GetValue(entity, null);
                if (obj == null)
                {
                    continue;
                }
                string value = obj.ToString(); //获取属性值
                ...
                else
                {
                    ...
                    {
                        param.Add(propertyInfo.Name, 1);
                    }
                    else
                    {
                        param.Add(propertyInfo.Name, 0);
                    }
                }
            }
            return param;
        ...
        private static void AppendAsc(StringBuilder sb, Type type, bool isasc = true)
        {
            List<PropertyInfo> keys = KeyPropertiesCache(type);
            if (keys.Count == 0)
            {
                keys = ExplicitKeyPropertiesCache(type);
                if (keys.Count == 0)
                {
                    return;
                }
...
public partial interface ISqlAdapter
{
    ...
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    void AppendColumnNameFuzzyEqualsValue(StringBuilder sb, string columnName);

    /// <summary>
    /// Adds a column not equality to a parameter.
    /// </summary>
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    void AppendColumnNameNotEqualsValue(StringBuilder sb, string columnName);
    ...
    void AppendPageInfo(StringBuilder sb, int pageIndex, int pageSize);
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    /// <summary>
    /// Adds a column like to a parameter.
    /// </summary>
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    public void AppendColumnNameFuzzyEqualsValue(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("[{0}] like '%'+@{1}+'%'", columnName, columnName);
    }
    /// <summary>
    /// Adds a column not equality to a parameter.
    /// </summary>
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    public void AppendColumnNameNotEqualsValue(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("[{0}] <> @{1}", columnName, columnName);
    }
    /// <summary>
    ...
    /// <param name="pageIndex"></param>
    /// <param name="pageSize"></param>
    public void AppendPageInfo(StringBuilder sb, int pageIndex, int pageSize)
    {
        sb.Insert(0, string.Format("select top {0} o.* from(select row_number() over(order by orderColumn) as rownumber, * from(", pageSize));
        sb.AppendFormat(" )as o where rownumber > {0}", pageIndex);
    }
}
...
public partial class MySqlAdapter : ISqlAdapter
{
    ...
    public void AppendColumnNameFuzzyEqualsValue(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("`{0}` like N'%+@{1}+%'", columnName, columnName);
    }
    ...
    public void AppendPageInfo(StringBuilder sb, int pageIndex, int pageSize)
    {
        ...
        sb.AppendFormat(" limit {0},{1}", start, pageSize);
    }
}
